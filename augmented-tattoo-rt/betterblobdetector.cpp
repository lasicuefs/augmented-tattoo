                                   ...
                                   int csign) const
{
    ...
    findBlobs(image, binaryImage, centers, curContours);
//    if(csign > 0) findBlobs(image, Scalar(255)-binaryImage, centers, curContours);
//    else findBlobs(image, binaryImage, centers, curContours);


//    std::cout << centers.size() << "-" << curContours.size() << std::endl;

    for (size_t i = 0; i < centers.size(); i++){
        Center &c = centers[i];
        keypoints.push_back(KeyPoint(c.location.x, c.location.y, c.radius));
    }
}
...
//        imshow("BinarizedImg", binarizedImage);
//        waitKey();
